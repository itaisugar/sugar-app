...
import { fetchBookmarkedItems } from '../lib/bookmarks';
...
export default function BookmarksScreen() {
  ...
  useEffect(() => {
    fetchBookmarkedItems().then(list => {
      setItems(list);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.topbar}>
        ...
        <Text style={TextStyles.overline}>Read Later</Text>
        <View style={{ width: 36 }} />
      </View>

      <View style={styles.headerBlock}>
        <Text style={TextStyles.kicker}>Saved For Later</Text>
        <Text style={styles.headerTitle}>{items.length}</Text>
        <Text style={[TextStyles.tagline, { marginTop: 4 }]}>
          Your private reading queue.
        </Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          <Text style={TextStyles.emptyTitle}>Nothing here yet</Text>
          <Text style={[TextStyles.emptyDescription, { textAlign: 'center', marginTop: 8 }]}>
            Tap the flag on any piece to add it to your reading queue.
          </Text>
        </View>
      ) : (
        <FlatList
