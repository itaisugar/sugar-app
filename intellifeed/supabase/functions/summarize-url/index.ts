...
- "summary": the article distilled into 5 or 6 short paragraphs, each separated by a blank line ("\\n\\n"). Each paragraph is rendered BOTH as a block of the article-reader body AND as ONE slide of the swipeable carousel — one paragraph IS one slide — so the golden rule is: each paragraph must be ONE complete, self-contained idea that never needs the next slide to make sense. A clear, finished thought matters more than a uniform length. Rules for every paragraph: it carries exactly ONE fact, insight, or conclusion and opens with its single sharpest point (lead with concrete numbers, names, or findings where the source has them); its length is FLEXIBLE between 3 and 5 lines of text — a crisp conclusion may be ~3 short lines (~15-20 words) while a richer explanation may run up to ~5 lines (~35-38 words), but NEVER exceed ~38 words, because anything longer overflows a single slide and gets cut. Do not pad a short idea to match a longer one; let it breathe. Keep sentences short and punchy, ideally 6-10 words, never a comma-heavy run-on (split a complex idea into two sentences). Structure the sequence as a story: the FIRST paragraph states the central finding or thesis; the MIDDLE paragraphs each deliver one piece of evidence, mechanism, data point, or the key caveat/counter-argument; the LAST paragraph is the takeaway — what it changes, why it matters, or what the reader should do with it. Voice: confident, measured, third-person, journalistic. Do not paraphrase the hook. No bullet lists, no headings, no second-person ("you").
